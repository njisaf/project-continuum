...
export async function remigrate(versionRange: { from: number; to?: number }): Promise<void> {
    if (!game.ready) {
        ui.notifications.warn("AVANT.Migrations.WorldNotReady", { localize: true });
        return;
    }
    if (game.user.role !== CONST.USER_ROLES.GAMEMASTER) {
        ui.notifications.error("AVANT.Migrations.OnlyGMCanUse", { localize: true });
        return;
    }
    ...
    if (migrations.length === 0 || versionRange.from < MigrationRunner.RECOMMENDED_SAFE_VERSION) {
        ui.notifications.error(
            game.i18n.format("AVANT.Migrations.OutsideSchemaRange", {
