import { ActorAvant, HazardAvant } from "@actor";
import { fontAwesomeIcon, htmlQuery } from "@util";
import type { XPCalculation } from "./index.ts";

function getHazards(actors: ActorAvant[]): HazardAvant[] {
    return actors.filter((a): a is HazardAvant => a.type === "hazard");
}

function getLevels(actors: ActorAvant[], alliance: string): number[] {
...
<table>
    <tr>
        <th>${game.i18n.localize("AVANT.Encounter.Budget.PartySize")}</th>
        <td>${xp.partySize}</td>
    </tr>
    <tr>
        <th>${game.i18n.localize("AVANT.Encounter.Budget.PartyLevel")}</th>
        <td>${xp.partyLevel}</td>
    </tr>
    <tr>
        <th>${game.i18n.localize("AVANT.Encounter.Budget.Threat")}</th>
        <td>${game.i18n.localize("AVANT.Encounter.Budget.Threats." + xp.rating)} (${xp.totalXP} XP)</td>
    </tr>
    <tr>
        <th>${game.i18n.localize("AVANT.Encounter.Budget.Reward")}</th>
        <td>${xp.xpPerPlayer} XP</td>
    </tr>
</table>
<h2>${game.i18n.localize("AVANT.Encounter.Budget.EncounterBudget")}</h2>
<table class="pf2-table">
    <tr>
        <th>${game.i18n.localize("AVANT.Encounter.Budget.Threat")}</th>
        <th>${game.i18n.localize("AVANT.Encounter.Budget.XPBudget")}</th>
        <th>${game.i18n.localize("AVANT.Encounter.Budget.XPNeeded")}</th>
        <th>${game.i18n.localize("AVANT.Encounter.Budget.Reward")}</th>
    </tr>
    <tr>
        <td>${game.i18n.localize("AVANT.Encounter.Budget.Threats.trivial")}</td>
        <td>${xp.encounterBudgets.trivial}</td>
        <td>${xp.encounterBudgets.trivial - xp.totalXP}</td>
        <td>40</td>
    </tr>
    <tr>
        <td>${game.i18n.localize("AVANT.Encounter.Budget.Threats.low")}</td>
        <td>${xp.encounterBudgets.low}</td>
        <td>${xp.encounterBudgets.low - xp.totalXP}</td>
        <td>60</td>
    </tr>
    <tr>
        <td>${game.i18n.localize("AVANT.Encounter.Budget.Threats.moderate")}</td>
        <td>${xp.encounterBudgets.moderate}</td>
        <td>${xp.encounterBudgets.moderate - xp.totalXP}</td>
        <td>80</td>
    </tr>
    <tr>
        <td>${game.i18n.localize("AVANT.Encounter.Budget.Threats.severe")}</td>
        <td>${xp.encounterBudgets.severe}</td>
        <td>${xp.encounterBudgets.severe - xp.totalXP}</td>
        <td>120</td>
    </tr>
    <tr>
        <td>${game.i18n.localize("AVANT.Encounter.Budget.Threats.extreme")}</td>
        <td>${xp.encounterBudgets.extreme}</td>
        <td>${xp.encounterBudgets.extreme - xp.totalXP}</td>
        <td>160</td>
    </tr>
</table>
<h2>${game.i18n.localize("AVANT.Encounter.CreatureXPAndRole.CreatureXPAndRole")}</h2>
<table class="pf2-table">
    <tr>
        <th>${game.i18n.localize("AVANT.Encounter.CreatureXPAndRole.CreatureLevel")}</th>
        <th>XP</th>
        <th>${game.i18n.localize("AVANT.Encounter.CreatureXPAndRole.SuggestedRole")}</th>
    </tr>
    <tr>
        <td>${xp.partyLevel - 4}</td>
        <td>10</td>
        <td>${game.i18n.localize("AVANT.Encounter.CreatureXPAndRole.CreatureLevels.-4")}</td>
    </tr>
    <tr>
        <td>${xp.partyLevel - 3}</td>
        <td>15</td>
        <td>${game.i18n.localize("AVANT.Encounter.CreatureXPAndRole.CreatureLevels.-3")}</td>
    </tr>
    <tr>
        <td>${xp.partyLevel - 2}</td>
        <td>20</td>
        <td>${game.i18n.localize("AVANT.Encounter.CreatureXPAndRole.CreatureLevels.-2")}</td>
    </tr>
    <tr>
        <td>${xp.partyLevel - 1}</td>
        <td>30</td>
        <td>${game.i18n.localize("AVANT.Encounter.CreatureXPAndRole.CreatureLevels.-1")}</td>
    </tr>
    <tr>
        <td>${xp.partyLevel}</td>
        <td>40</td>
        <td>${game.i18n.localize("AVANT.Encounter.CreatureXPAndRole.CreatureLevels.0")}</td>
    </tr>
    <tr>
        <td>${xp.partyLevel + 1}</td>
        <td>60</td>
...
const askLevelPopupTemplate = (): string => {
    ...
    <div class="form-group">
        <label>${game.i18n.localize("AVANT.Encounter.Budget.PartySize")}</label>
        <input id="party-size" name="party-size" type="number" value="${partySize}">
    </div>
    <div class="form-group">
        <label>${game.i18n.localize("AVANT.Encounter.Budget.PartyLevel")}</label>
...
function showXP(partyLevel: number, partySize: number, npcLevels: number[], hazards: HazardAvant[]): void {
    const pwol = game.avant.settings.variants.pwol.enabled;
    const xp = game.avant.gm.calculateXP(partyLevel, partySize, npcLevels, hazards, { pwol });
    new Dialog({
        title: "XP",
...
function askPartyLevelAndSize(npcLevels: number[], hazards: HazardAvant[]): void {
    new Dialog({
