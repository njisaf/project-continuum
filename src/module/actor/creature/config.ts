...
import type { CreatureAvant } from "./document.ts";

/** A DocumentSheet presenting additional, per-actor settings */
abstract class CreatureConfig<TActor extends CreatureAvant> extends DocumentSheet<TActor> {
    override get title(): string {
        const namespace = this.actor.isOfType("character") ? "Character" : "NPC";
        return game.i18n.localize(`AVANT.Actor.${namespace}.Configure.Title`);
    }

    override get template(): string {
        return `systems/avant/templates/actors/${this.actor.type}/config.hbs`;
    }
    ...
    override async getData(options: Partial<DocumentSheetOptions> = {}): Promise<CreatureConfigData<TActor>> {
        ...
        const defaultValue = game.i18n.localize(
            this.actor.hasPlayerOwner
                ? "AVANT.Actor.Creature.Alliance.Party"
                : "AVANT.Actor.Creature.Alliance.Opposition",
        );

        const allianceOptions = {
            default: game.i18n.format("AVANT.Actor.Creature.Alliance.Default", { alliance: defaultValue }),
            opposition: "AVANT.Actor.Creature.Alliance.Opposition",
            party: "AVANT.Actor.Creature.Alliance.Party",
            neutral: "AVANT.Actor.Creature.Alliance.Neutral",
        };
    ...
    override async _updateObject(event: Event, formData: Record<string, unknown>): Promise<void> {
        ...
        if (alliance === "default") {
            ...
            throw ErrorAvant("Unrecognized alliance");
        }
...
interface CreatureConfigData<TActor extends CreatureAvant> extends DocumentSheetData<TActor> {
